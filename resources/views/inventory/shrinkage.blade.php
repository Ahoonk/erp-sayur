...
<div class="space-y-6 max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div>
        <h1 class="text-2xl font-semibold text-gray-800">Shrinkage / Waste</h1>
        <p class="text-sm text-gray-500 mt-1">Qty shrinkage mengikuti stok tersedia dari hasil stok masuk.</p>
    </div>

    @php($submitFormId = 'shrinkage-form')
    @php($submitLabel = 'Catat Shrinkage')
    @include('inventory._tabs')

    <div class="bg-white rounded-xl shadow p-5">
        <form id="shrinkage-form" action="{{ route('inventory.shrinkage') }}" method="POST" class="space-y-3 text-sm"
            x-data='{
