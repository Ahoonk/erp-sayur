...
<div class="space-y-6 max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div>
        <h1 class="text-2xl font-semibold text-gray-800">Stok Keluar</h1>
        <p class="text-sm text-gray-500 mt-1">Qty keluar mengikuti stok tersedia dari hasil stok masuk (FEFO).</p>
    </div>

    @php($submitFormId = 'stock-out-form')
    @php($submitLabel = 'Simpan Stock Out')
    @include('inventory._tabs')

    <div class="bg-white rounded-xl shadow p-5">
        <form id="stock-out-form" action="{{ route('inventory.stock-out') }}" method="POST" class="space-y-3 text-sm"
            x-data='{
